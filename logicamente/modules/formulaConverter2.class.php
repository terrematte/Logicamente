...
class formulaConverter {
	...
	private function  makeTree ($expression, $arrayLinked, $arrayUnlinked, $count, $count1, $arrayQuantifier){
		...
		if ($this->isQuantifier($op)) {
			...
			$var =	new Variable(substr($op,$index));
			$var->isLinked = true;
			$var->value = rand(0,1000);
			$node = new Node(new Quantifier(substr($op,0,$index), $var));
			$count1++;
			array_push($arrayQuantifier[$count],$count1);
			$arrayLinked[$count1] = Array((substr($op,$index)) => $var);
		...
		/*Testa se op é um conectivo*/
			...
			for ($i = sizeof($array['children']) - 1; $i >= 0; $i--) {
				...
				$children = $this->makeTree($child,$arrayLinked, $arrayUnlinked, $count, $count1,$arrayQuantifier);
	...
	public function infixToTree($expression,$type) {
		...
		if ($this->testParentheses($expression)) {
			...
			if ($type) {
				...
				$arrayAtoms = $this->arrayAtoms($expression);
				return $this->propMakeTree($expression, $arrayAtoms);
			}
		} else return "";
	}
	...
	private function firstToArray($expression,$arrayExpression) {
		...
		if ($expression != "") {
			...
			/*Testa se o caractere é um ','*/
				...
				if ($expression[$indexEnd] == '~') {
					if ($indexEnd == 0)
						$arrayExpression[count($arrayExpression)-1] = $expression[$indexEnd];
					else
						array_push($arrayExpression,$expression[$indexEnd]);
					$indexEnd++;
				}
	...
	public function printTree ($tree,$ident) {
		...
		if ($node instanceof Quantifier) {
			echo $ident."<i><b>Quantifier Object</b></i><br />";
			echo $ident."[<i>content</i>] => $node->content<br />";
			echo $ident."[<i>bound_variable</i>] => <br />";
			echo "<DL>";
			$ident2 = $ident . "<DD>";
			echo $ident2."<i><b>Variable of Quantifier</b></i><br />";
			$node2 = $node->bound_variable;
			echo $ident2."[<i>content</i>] => $node2->content <br />";
			echo $ident2."[<i>value</i>] => $node2->value <br />";
			echo $ident2."[<i>isLinked</i>] => $node2->isLinked <br />";
			echo "</DL>";
		...
		/*Testa se é uma variável*/
			...
			echo $ident."[<i>content</i>] => $node->content<br />";
			echo $ident."[<i>value</i>] => $node->value<br />";
			echo $ident."[<i>isLinked</i>] => $node->isLinked<br />";
		/*Testa se é uma constante*/
	 ...
	 * @return node o nó raiz da árvore pronta.
	 */
	private function propMakeTree($expression, $arrayAtoms) {
		$array = $this->connectiveChildren($expression);
		$connective = $array['connective'];
		/*Testa se op é um conectivo*/
		if ($this->isConnective($connective)) {
			...
			for ($i = sizeof($array['children']) - 1; $i >= 0; $i--) {
				$arity++;
				$child = $array['children'][$i];
				$children = $this->propMakeTree($child, $arrayAtoms);
				array_push($node->children, $children);
			}
			$tree->arity = $arity;
			$node->content = $tree;
		...
		/*Caso contrário é um átomo*/
			...
			$tree = new Node ($arrayAtoms[$connective]);
			return $tree;
		}
	}


	private function arrayAtoms($expression) {
		$tmp = ($this->connectiveChildren($expression));
		$array = $tmp['children'];
		$arrayFinal = Array();
		if ($this->isRelation($tmp['connective'])) {
			array_push($array,$tmp['connective']);
		}
		/*Laço para pegar todas as variáveis e constantes existentes na expressão
		 e colocá-las em um array*/
		while ($tmp = array_pop($array)) {
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////
	/////////////////////////////////////////////////////////////////////////////////////////////////////
	public function printFormula ($tree,$string) {
		/*Testa se é um conectivo*/
		$node = $tree->content;
		$node2;
		if ($node instanceof Quantifier) {
			$string .= $node->content;
			$node2 = $node->bound_variable;
			$string .= $node2->content . " ";
			/*Laço que chama a recursão para cada argumento*/
			foreach ($tree->children as $term) {
				$string = $this->printFormula($term,$string);
			}
		/*Testa se é um conectivo*/
		} elseif ($node instanceof Connective) {
			...
			$string = substr_replace($string, ")", (-2 - strlen($node->content)));
		/*Testa se é uma relação*/
		} elseif ($node instanceof Relation) {
			$string .= $node->content . "(";
			/*Laço que chama a recursão para cada argumento*/
	...
	public function functionToTree($expression) {
		/*Testa se existe a mesma quantidade de '(' e ')'*/
		if ($this->testParentheses($expression)) {
			if ($this->isVariable($expression)) {
				$node = new Node (new Variable($expression));
			} elseif ($this->isConstant($expression)) {
				$node = new Node (new Constant($expression));
			} else {
				$arrayLinked = Array(Array());
				$arrayUnlinked = $this->makeArrayUnlinked($expression);
				$arrayQuantifier = Array(array());
				$node = $this->makeTree2($expression,$arrayLinked,$arrayUnlinked,0,0,$arrayQuantifier);
			}
			return $node;
			} else return "";
	}
